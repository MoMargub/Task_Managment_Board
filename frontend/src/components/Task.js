...
const Task = () => {
  ...
  useEffect(() => {
    if (!isAddTaskModalOpen || isRenderChange) {
      axios
        .get(`${process.env.REACT_APP_BASE_URI}/project/${projectId}`)
        .then((res) => {
  ...
  const updateTodo = (data) => {
    axios
      .put(`${process.env.REACT_APP_BASE_URI}/project/${projectId}/todo`, data)
      .then((res) => {})
      .catch((error) => {
        toast.error("Something went wrong");
      });
  ...
  return (
    <div className="px-12 py-6 w-full">
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className="pb-2">
                                        ...
                                        <span
                                          className="ml-2 py-1 px-2.5 bg-indigo-100 text-white rounded-md text-xs font-medium mt-1 inline-block"
                                          style={{
                                            backgroundColor:
                                              item.priority === "Low"
                                                ? "green"
      ...
      <AddTaskModal
        ...
        projectId={projectId}
      />
      <TaskModal isOpen={isTaskOpen} setIsTaskOpen={setTaskOpen} id={taskId} />
    </div>
  );
};
