...
const Sidebar = () => {
  ...
  const projectData = () => {
    axios.get(`${process.env.REACT_APP_BASE_URI}/projects`).then((res) => {
  ...
  return (
    <div className="py-5">
      <div className="px-4 mb-3 flex items-center justify-between">
        ...
        <button
          onClick={openModal}
          className="bg-indigo-200 rounded-full p-[2px] focus:outline focus:ring focus:ring-indigo-200 focus:ring-offset-1"
        >
          <svg
      ...
      <ul className="border-r border-gray-300 pr-2">
        {projects.map((project, index) => (
          ...
          >
            <li
              className={`px-5 py-1 mb-1 text-gray-600 font text-sm capitalize select-none hover:text-indigo-600 rounded transition-colors hover:bg-indigo-200/80 ${
